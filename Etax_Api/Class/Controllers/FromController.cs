...
namespace Etax_Api.Controllers
{
    ...
    public class FromController : ControllerBase
    {
        ...
        public FromController(ApplicationDbContext context, IConfiguration config, IExceptionLogger exceptionLogger, IJwtService jwtService)
        {
            _config = config;
            _context = context;
		...
		//*New for Koomkoom
        [HttpPost]
        [Route("get_data_form_kk")]
        public async Task<IActionResult> GetDataForm_Koomkoom([FromBody] BodyMkKoomkoomData bodyKkData)
        {
            try
            {
                DateTime now = DateTime.Now;
                var branchs = await (from b in _context.branchs
                                     where b.name.Contains(bodyKkData.branchID)
                                     select new
                                     {
                                         b.id,
                                         b.member_id,
				...
				//Checksum Function
				string TextChecksum_X = bodyKkData.branchID + bodyKkData.billID.Split('-').Last() + bodyKkData.amount + bodyKkData.noDiscount + bodyKkData.fAndB + bodyKkData.service + bodyKkData.totalBeforeRounding + bodyKkData.rounding;
				string TextChecksum_Y = bodyKkData.billDate + bodyKkData.billID.Split('-').Last() + bodyKkData.discount + bodyKkData.totalAmount + bodyKkData.vat + bodyKkData.baseAmount + bodyKkData.billIDRef.Split('-').Last();
				string dataX = "";
				string dataY = "";
				MatchCollection matchX = rg.Matches(TextChecksum_X);
				if (matchX.Count > 0)
				...
				// Console.WriteLine("dataX: " + dataX);
                // Console.WriteLine("dataY: " + dataY);
				// Console.WriteLine("checksum: " + checksum);
                // Console.WriteLine("bodyKkData.checkSum: " + bodyKkData.checkSum);
				
                if (checksum != bodyKkData.checkSum)
					return StatusCode(404, new
					{
						message = "รูปแบบข้อมูลไม่ถูกต้อง",
					});

                //Read data form etax_files to show company details
                var etaxFile = await
                    (from ef in _context.etax_files
                     where ef.member_id == branch.member_id &&
                     ef.ref_etax_id == bodyKkData.billID &&
                     ef.other.Contains(bodyKkData.branchID + "|")
                     select new
                     {
                         ef.id,
                         ef.etax_id,
                ...
                //If TIR reciept
                EtaxFile etaxFile_tir = null;
                if (bodyKkData.option.ToUpper() == "TIR")
                {
                    //หารายการที่มีการทำใบแทน
                    etaxFile_tir = await _context.etax_files
                                           .Where(x =>
                ...
                var item = new { name = "อาหารและเครื่องดื่ม" };
                if (bodyKkData.option == "food")
                {
                    food_edit = true;
                }

                //Check invoice expiration
                if (etaxFile != null) //If etax_files is available in the database
                {
                    string[] otherArray = etaxFile.other.Split('|');
                    if (otherArray.Length >= 4)
                    {
                    ...
                    if (bodyKkData.branchID == "D201") //If from member card(?)
                    {
                        expiryDate = DateTime.ParseExact(now.ToString("yyyy-MM") + "-01", "yyyy-MM-dd", CultureInfo.InvariantCulture);
                        ref_issue_date = DateTime.ParseExact(((DateTime)etaxFile.ref_issue_date).ToString("yyyy-MM") + "-01", "yyyy-MM-dd", CultureInfo.InvariantCulture);
                    ...
                    else
                    {
                        ...
                        if (bodyKkData.option != "over" && expiryDate >= ref_issue_date)
                            expire_cancel = true;
                    }


                    //Check delete_status if this invoice is canceled
                    if (etaxFile.delete_status == 1)
                        cancel = true;

                    //Check add_ebxml_status if this invoice is send to RD
                    if (etaxFile.add_ebxml_status == "success")
                        rd = true;
                ...
                //if(etaxFile_tir != null) //If this is TIR - may have additional logic but nothing special for now

                if (member != null && branch != null)//If member and branch is found
                {
                    string branch_name = "";
                    string[] branchNameArray = branch.name.Split('|');
                    foreach (string branchName in branchNameArray)
                    {
                        if (branchName.Contains(bodyKkData.branchID))
                        {
                            branch_name = branchName;
                            break;
                        }
        ...
        [HttpPost]
        [Route("save_form_TIR_mk")]
        public async Task<IActionResult> SaveFormTIR_MK([FromBody] BodyMkUserform bodyUserform)
        {
            try
            {
                DateTime now = DateTime.Now;
                ...
                //หารายการที่มีการทำใบแทน
                EtaxFile etaxFile_tir = await _context.etax_files
                                    .Where(x =>
                                    x.member_id == etaxFile.member_id &&
                                    x.etax_id.StartsWith(etaxFile.etax_id) &&
                                    x.document_type_id == 9 &&
                                    x.delete_status == 0
                                    ).OrderByDescending(x => x.etax_id)
                ...
                bool rd_tir = false; //ไว้เช็คว่าใบแทนมีการส่งสรรพากรแล้วหรือยัง
                        ...
                        if (bodyUserform.lang == "EN")
                        {
                            using (var transaction = _context.Database.BeginTransaction())
                            {
                                try
                                {
                                    ...
                                    etaxFile_tir.gen_xml_status = "pending";
                                    etaxFile_tir.gen_pdf_status = "pending";
                                    etaxFile_tir.add_email_status = "pending";
                                    etaxFile_tir.buyer_name = bodyUserform.name;
                                    etaxFile_tir.buyer_tax_id = bodyUserform.tax_id;
                                    etaxFile_tir.buyer_address = $"{bodyUserform.address} {bodyUserform.district} {bodyUserform.amphoe} {bodyUserform.province}";
                                    etaxFile_tir.buyer_zipcode = bodyUserform.zipcode;
                                    etaxFile_tir.buyer_tel = bodyUserform.tel;
                                    etaxFile_tir.buyer_email = bodyUserform.email;
                                    etaxFile_tir.remark = bodyUserform.remark;
                                    etaxFile_tir.other = $"{bodyUserform.dataQr.branchCode}|EN|{bodyUserform.type}|{bodyUserform.dataQr.url}";
                                    ...
                                    etaxFile_tir.update_date = DateTime.Now;

                                    if (bodyUserform.type == "LE")
                                    {
                                        etaxFile_tir.buyer_branch_code = bodyUserform.branch_code;
                                    }
                    ...
                    else
                    {
                        ...
                        if (bodyUserform.lang == "EN")
                        {
                            if (etaxFile != null)
                            {
                                ...
                                string[] parts = etaxFile_tir.etax_id.Split('-');
                                ...
                                using (var transaction = _context.Database.BeginTransaction())
                                {
                                    try
                                    {
                                        EtaxFile newEtaxFile = new EtaxFile()
                                        {
                                            ...
                                            etax_id = new_etax_id,
                                            issue_date = now,
                                            ref_etax_id = etaxFile_tir.etax_id,
                                            ref_issue_date = etaxFile_tir.issue_date,
                                            buyer_branch_code = etaxFile_tir.buyer_branch_code,
                                            buyer_id = "",
                                            buyer_name = bodyUserform.name,
                                            buyer_tax_id = bodyUserform.tax_id,
                                            buyer_tax_type = "",
                                            ...
                                            buyer_country_code = "TH",
                                            buyer_email = bodyUserform.email,
                                            price = etaxFile_tir.price,
                                            discount = etaxFile_tir.discount,
                                            tax_rate = 7,
                                            tax = etaxFile_tir.tax,
                                            total = etaxFile_tir.total,
                                            remark = bodyUserform.remark,
                                            other = bodyUserform.dataQr.branchCode + "|EN|" + bodyUserform.type + "|" + bodyUserform.dataQr.url,
                                            other2 = bodyUserform.dataQr.baseAmount + "|" + bodyUserform.dataQr.noDiscount + "|" + bodyUserform.dataQr.fAndB + "|" + bodyUserform.dataQr.service,
                                            group_name = "",
                                            template_pdf = "",
                            ...
                            using (var transaction = _context.Database.BeginTransaction())
                                {
                                    try
                                    {
                                        EtaxFile newEtaxFile = new EtaxFile()
                                        {
                                            member_id = etaxFile_tir.member_id,
                                            branch_id = etaxFile_tir.branch_id,
                                            member_user_id = 0,
                                            document_type_id = 9,
                                            create_type = "api",
                                            rawdata_file_id = 0,
                ...
                else
                {
                    ...
                    if (bodyUserform.lang == "EN")
                    {
                        if (etaxFile != null)
                        {
                            ...
                            string etax_id = bodyUserform.dataQr.billIDRef;
                                etax_id = etax_id + "-" + "00";
                            ...
                            using (var transaction = _context.Database.BeginTransaction())
                            {
                                try
                                {
                                    EtaxFile newEtaxFile = new EtaxFile()
                                    {
                                        ...
                                        etax_id = etax_id,
                                        issue_date = now,
                                        ref_etax_id = etaxFile.etax_id,
                                        ref_issue_date = etaxFile.issue_date,
                                        buyer_branch_code = etaxFile.buyer_branch_code,
                                        buyer_id = "",
                                        buyer_name = bodyUserform.name,
                                        buyer_tax_id = bodyUserform.tax_id,
                                        buyer_tax_type = "",
            ...
            {
                return StatusCode(400, new { message = "บันทึกข้อมูลไม่สำเร็จ กรุณาลองใหม่ภายหลังหรือติดต่อเจ้าหน้าที่" });
            }
        }
