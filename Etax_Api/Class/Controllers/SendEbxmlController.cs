...
namespace Etax_Api.Controllers
{
    ...
    public class SendEbxmlController : ControllerBase
    {
        ...
        public SendEbxmlController(ApplicationDbContext context, IConfiguration config, IExceptionLogger exceptionLogger, IJwtService jwtService)
        {
            _config = config;
            _context = context;
        ...
        public async Task<IActionResult> GetSendEbxmlDataTabel([FromBody] BodyDtParameters bodyDtParameters)
        {
            try
            {
                ...
                if (permission.per_ebxml_view != "Y")
        ...
        public async Task<IActionResult> GetPdfDataDetail(int id)
        {
            try
            {
                ...
                var permission = await (from mup in _context.member_user_permission
                                        where mup.member_user_id == jwtStatus.user_id
                                        select mup.per_ebxml_view).FirstOrDefaultAsync();
                if (permission != "Y")
                    return StatusCode(401, new { message = "ไม่มีสิทธิในการใช้งานส่วนนี้", });
        ...
        public async Task<IActionResult> GetSendEbxmlDataTabelAdmin([FromBody] BodyAdminDtParameters bodyDtParameters)
        {
            try
            {
                string token = Request.Headers[HeaderNames.Authorization].ToString();
        ...
        public async Task<IActionResult> GetPdfDataDetailAdmin(int id)
        {
            try
            {
                ...
                var permission = await (from up in _context.user_permission
                                        where up.user_id == jwtStatus.user_id
                                        select up.per_ebxml_detail).FirstOrDefaultAsync();
                ...
                var membere = await (from um in _context.user_members
                                     where um.user_id == jwtStatus.user_id && um.member_id == sendEbxml.member_id
