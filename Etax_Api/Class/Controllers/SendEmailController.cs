    ...
    public class SendEmailController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetSendEbxmlDataTabelAdmin([FromBody] BodyDtParameters bodyDtParameters)
        {
            try
            {
                ...
                var result = _context.view_send_email.Where(x => x.member_id == bodyDtParameters.id).AsQueryable();
                ...
                var filteredResultsCount = await result.CountAsync();
                var totalResultsCount = await _context.view_send_email.Where(x => x.member_id == bodyDtParameters.id).CountAsync();
