...
namespace Etax_Api.Controllers
{
    ...
    public class EtaxFilesController : ControllerBase
    {
        ...
        public EtaxFilesController(ApplicationDbContext context, IConfiguration config, IExceptionLogger exceptionLogger, IJwtService jwtService)
        {
            _config = config;
            _context = context;
        ...
        public async Task<IActionResult> GetXmlFilesDataTabel([FromBody] BodyDtParameters bodyDtParameters)
        {
            try
            {
                ...
                var result = _context.view_etax_files.Where(x => x.member_id == jwtStatus.member_id && listDocumentTypeID.Contains(x.document_type_id) && x.delete_status == 0).AsQueryable();
        ...
        public async Task<IActionResult> GetAddXmlDetail()
        {
            try
            {
                ...
                var permission = await (from mup in _context.member_user_permission
                                        where mup.member_user_id == jwtStatus.user_id
                                        select mup.per_xml_manage).FirstOrDefaultAsync();
                if (permission != "Y")
                    return StatusCode(401, new { message = "ไม่มีสิทธิในการใช้งานส่วนนี้", });
        ...
        public async Task<IActionResult> GetXmlDataDetail(int id)
        {
            try
            {
                ...
                var permission = await (from mup in _context.member_user_permission
                                        where mup.member_user_id == jwtStatus.user_id
                                        select mup.per_xml_view).FirstOrDefaultAsync();
                if (permission != "Y")
                    return StatusCode(401, new { message = "ไม่มีสิทธิในการใช้งานส่วนนี้", });
        ...
        public async Task<IActionResult> GetPdfDataDetail(int id)
        {
            try
            {
                ...
                var permission = await (from mup in _context.member_user_permission
                                        where mup.member_user_id == jwtStatus.user_id
                                        select mup.per_pdf_view).FirstOrDefaultAsync();
                if (permission != "Y")
                    return StatusCode(401, new { message = "ไม่มีสิทธิในการใช้งานส่วนนี้", });
        ...
        public async Task<IActionResult> Delete(int id)
        {
            try
            {
                ...
                var permission = await (from mup in _context.member_user_permission
                                        where mup.member_user_id == jwtStatus.user_id
                                        select mup.per_etax_delete).FirstOrDefaultAsync();
                if (permission != "Y")
                    return StatusCode(401, new { message = "ไม่มีสิทธิในการใช้งานส่วนนี้", });
        ...
        public async Task<IActionResult> ImportEmail([FromBody] BodyImportEmail bodyImportEmail)
        {
            try
            {
                ...
                var permission = await (from mup in _context.member_user_permission
                                        where mup.member_user_id == jwtStatus.user_id
                                        select mup.per_email_import).FirstOrDefaultAsync();
                if (permission != "Y")
                    return StatusCode(401, new { message = "ไม่มีสิทธิในการใช้งานส่วนนี้", });


                string extension = Path.GetExtension(bodyImportEmail.email_file_name);
                if (extension != ".xlsx" && extension != ".xls")
                {
                    return StatusCode(400, new { message = "สามารถ upload ไฟล์ .xls .xlsx ได้เท่านั้น", });
                }
